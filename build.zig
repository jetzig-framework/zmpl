...
pub fn build(b: *Build) !void {
    ...
    const templates_paths: []const []const u8 = if (templates_path) |path|
        ...
        templates_paths_option orelse try templatesPaths(b.allocator, &.{.{
            .prefix = "templates",
            .path = &.{ "src", "templates" },
        }});
    ...
    const manifest_module = b.addModule("zmpl.manifest", .{
    ...
    manifest_module.addImport("zmpl", zmpl);
    manifest_module.addImport("zmd", zmd);
    zmpl.addImport("zmpl.manifest", manifest_module);
...
fn generateZmplOptions(
    ...
    const encodedHeader: []u8 = try allocator.alloc(u8, Encoder.calcSize(manifest_header.len));
    ...
    const template =
        ...
        \\
    ;
    return std.fmt.allocPrint(allocator, template, .{
