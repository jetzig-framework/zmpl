...
pub fn compile(
    ...
    var output_dir_handle = try std.fs.cwd().makeOpenPath(output_dir, .{});
    ...
    // First pass - generate simple names for all templates (no hashing)
    for (self.template_paths) |template_path| {
        ...
        // Use the template key as the name directly (no hashing)
        ...
        try map.putNoClobber(allocator, key, simple_name);
    }

    // Second pass - compile all templates to individual files
    ...
    std.debug.print("[zmpl] Compiled {} template(s) to individual files\n", .{self.template_paths.len});

    // Generate the main manifest file that imports all templates
    try self.writeManifest(allocator, output_dir_handle, &template_defs, options);
}
...
fn compileTemplates(
    ...
    for (self.template_paths) |template_path| {
        ...
        const simple_name = template_map.get(template_path.prefix).?.get(key).?;
        ...
        // Write this template to its own file
        const template_filename = try std.fmt.allocPrint(allocator, "{s}.zig", .{simple_name});
        // Note: Don't free template_filename - it's stored in template_def

        const template_file = try zmpl_dir.createFile(template_filename, .{});
        defer template_file.close();

        try template_file.writeAll(output);
...
fn writeManifest(
    ...
    template_defs: *const ArrayList(TemplateDef),
    comptime options: type,
) !void {
    _ = self;

    const manifest_file = try output_dir.createFile("zmpl.manifest.zig", .{});
    defer manifest_file.close();

    var buffer: [8192]u8 = undefined;
    var writer = manifest_file.writerStreaming(&buffer);
    ...
    // Write optional manifest header
    ...
    for (template_defs.items) |template_def| {
        try w.print("    pub const {s} = @import(\"{s}\");\n", .{
            template_def.name,
            template_def.filename,
        });
    }

    try w.writeAll(
        \\
        \\    // Re-export block functions for easy access
        \\
    );

    // Re-export all block functions
    for (template_defs.items) |template_def| {
        var it = template_def.blocks.valueIterator();
        while (it.next()) |block_list| {
            for (block_list.items) |block| {
                try w.print("    pub const {s} = @This().{s}.{s};\n", .{
                    block.func,
                    template_def.name,
                    block.func,
                });
    ...
    // Write template array entries (non-partials only)
    for (template_defs.items) |template_def| {
        ...
        try template_def.renderBlocks(w);
