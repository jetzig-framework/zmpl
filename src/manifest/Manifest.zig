...
pub fn compile(
    ...
    const template_defs = try self.allocator.create(ArrayList(TemplateDef));
    template_defs.* = .empty;

    var templates_paths_map: StringHashMap([]const u8) = .init(self.allocator);
    for (self.templates_paths) |templates_path| {
        try templates_paths_map.put(templates_path.prefix, templates_path.path);
    }

    var template_map: StringHashMap(TemplateType.TemplateMap) = .init(self.allocator);
...
fn compileTemplates(
    ...
    comptime TemplateType: type,
    template_map: *StringHashMap(TemplateType.TemplateMap),
    comptime options: type,
) !void {
    for (self.template_paths) |template_path| {
        ...
        try array.append(self.allocator, template_def);
    }
}
...
const blueprint = struct {
    block1: []const u8 =
        \\// Zmpl template manifest.
        \\// This file is automatically generated at build time and should not be
        \\// manually modified.
        \\
        \\const std = @import("std");
        \\const __zmpl = @import("zmpl");
    ...
    block4: []const u8 =
        \\    };
        \\
        \\    /// Find any template matching a given name. Uses all template paths
        \\    /// in order.
        \\    pub fn find(name: []const u8) ?Template {
        \\        for (templates) |template| {
        \\            if (!std.mem.eql(u8, template.key, name)) continue;
        ...
        \\    }
        \\
        \\    /// Find a template in a given prefix, i.e. a template located
        \\    /// within a specific template path.
        \\    pub fn findPrefixed(prefix: []const u8, name: []const u8) ?Template {
        \\        for (templates) |template| {
        \\            if (!std.mem.eql(u8, template.prefix, prefix)) continue;
        \\            if (!std.mem.eql(u8, template.key, name)) continue;
